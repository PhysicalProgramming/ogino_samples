...
void draw () {
  ...
  for ( int i = 0; i < 3; i++ ) {
    ...
    float z = map( sensorData[2], -3.0, 3.0, 0, 1.0 );    
    
    int X_NUM = 20;
    int Y_NUM = 20;
    int margin = 10;
    try {
      margin = width / int((float)X_NUM * z);
    }
    catch ( Exception e ) {
    }
    int xPos = 0;
    int yPos = 0;
    for ( int xLine = 0; xLine < X_NUM; xLine++ ) {
      
      xPos = xLine * margin - width/2 + margin/2;
      
      for ( int yLine = 0; yLine < Y_NUM; yLine++ ) {
        
        yPos = yLine * margin - height/2 + margin/2;
      
        // ブラーなし
        drawCircle( (int)x + xPos, (int)y + yPos, (int)CIRCLE_SIZE + int((1.0-z)*100.0) );
