...
void draw() {
  ...
  line( pointA_Left, positionTop+5,  pointA_Left+lineSize + waveSize*sin(frequency), positionTop-5 );
  ...
  // 時計の秒数に合わせて縦線を加える
  float radius = millis();
  line( centerLeft, centerTop, centerLeft + secondCircleSize*cos(radius), centerTop + secondCircleSize*sin(radius) );
