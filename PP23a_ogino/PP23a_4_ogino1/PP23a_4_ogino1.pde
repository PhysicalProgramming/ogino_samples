...
ArrayList<PVector> linePositions = new ArrayList<PVector>();
...
void AddLine () {
	...
	PVector lineSpeed = new PVector( 0, random(1, 4) );
	lineSpeeds.add( lineSpeed );

	PVector linePosition = new PVector( -1.0, 0 );
	linePositions.add( linePosition );

	Float alpha = random( 50, 255 + sensorData[2]*100.0 );
	alphas.add( alpha );
	...
	for ( int pointPosX = 0; pointPosX <= width; pointPosX+=2 ) {
		...
		//newLine.add( new PVector(pointPosX-width/2, -height*0.25 + cos( (pointPosX + millis()) * 0.01 * lineSpeed.y ) * 10 ) );
		newLine.add( new PVector(pointPosX-width/2, -height*0.25 + cos( (pointPosX + millis()) * 0.04 * sensorData[2] ) * 10 ) );
...
void DrawLines () {
	...
	float rot = atan2( sensorData[0] * PI, sensorData[1] * PI );
	//float rot = atan2( 0 + sensorData[0], lineSpeed.x * sensorData[1] ) * 0.1;
	...
	for ( int lineNumber = 0; lineNumber < lines.size(); lineNumber++ ) {
		...
		PVector linePosition = linePositions.get(lineNumber);
		lineSpeeds.set( lineNumber, lineSpeed );

		linePosition.x += sensorData[0] * 0.01;
		linePosition.y += lineSpeed.y;// + sensorData[1];
		linePositions.set( lineNumber, linePosition );

		// https://processing.org/reference/stroke_.html
		stroke(alphas.get(lineNumber));

		ArrayList<PVector> currentLine = lines.get(lineNumber);
		for ( int pointNumber = 0; pointNumber < currentLine.size(); pointNumber++ ) {

			//PVector updatePoint = PVector.add( currentLine.get(pointNumber), new PVector( 0, 0 ) );
			//PVector updatePoint = PVector.add( currentLine.get(pointNumber), new PVector( 0, lineSpeed.x ) );
			//PVector updatePoint = PVector.add( currentLine.get(pointNumber), new PVector( 0 + sensorData[0], lineSpeed.x * sensorData[1] ) );
			...
			if ( pointNumber > 0 ) {
				pushMatrix();
				//scale( 3.0, 1.0 );
				scale( linePosition.x, 1.0 );
				//translate( linePosition.x, linePosition.y );
				translate( 0, linePosition.y );
				//rotate( rot );
		...
		if ( linePosition.y > height * 1.5 ) {

			lines.remove( lineNumber );
			lineSpeeds.remove( lineNumber );
			linePositions.remove( lineNumber );
